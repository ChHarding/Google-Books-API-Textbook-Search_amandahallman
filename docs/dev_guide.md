...
## Overview
This application is a web-based tool designed for users to search for textbooks by subject, title, author, or ISBN. It uses the Google Books API to search for textbooks based on user queries and is also designed for uploading PDF syllabi and extracting textbook information using OpenAI's GPT-3. It leverages Flask for web server management, Jinja2 for templating, and PDFMiner for PDF text extraction.
